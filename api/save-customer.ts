...
export default async function handler(req: any, res: any) {
  ...
  const { name, email, phone, street, city, state, zip, wcCustomerId: bodyCustomerId, newPassword } = req.body || {};
  ...
  if (newPassword) {
    ...
    try {
      ...
      const updateRes = await fetch(`${wcUrl}/wp-json/wc/v3/customers/${customers[0].id}`, {
        method: 'PUT',
        headers: { Authorization: authHeader, 'Content-Type': 'application/json' },
        body: JSON.stringify({ password: newPassword }),
      });
      if (!updateRes.ok) return res.status(502).json({ error: 'Failed to update password' });
      return res.json({ success: true });
    } catch (err) {
      console.error('[save-customer] password update exception:', err);
